...
func _ready():
	if boss == true:
		$PlayerSprite.self_modulate = Color.BLACK
		health_max = 300
		speed = 110
		self.scale = Vector2(1.20,1.20)
	health = health_max
	...
	if can_take_damage:
		...
		if alive:
			if global.player_fire_upgrade and area.fire:
				...
				$FireDamage.start()
			if boss == false:
				$PlayerSprite.self_modulate = Color.BLUE
			else:
