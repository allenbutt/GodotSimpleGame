...
func _attack_target():
	...
	if randi_range(0,1) <= 1 and melee_distance:
		attack_animation_name = "side_attack"
#	elif randi_range(0,0) == 0:
#		attack_animation_name = "side_attack2"
	else:
...
func ranged_attack():
	...
	var projectile_instance = skeleton_projectile.instantiate()
	projectile_instance.position = shoot_position.global_position
	projectile_instance.direction = Vector2(player.position.x-position.x+randf_range(-20,20),player.position.y-position.y+randf_range(-20,40)).normalized()
	if boss:
